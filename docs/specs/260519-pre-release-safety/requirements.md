...
Close four pre-v0.1.0 safety gaps surfaced by a package-wide review:
silent basis mismatch on reload, missing spin unit-vector validation,
loss of `torque_weight` on persistence, and missing integration coverage
for `SCEModel`, batch `predict_*`, and cross-basis mismatch detection.
...
- `torque_weight` (a fit-time hyperparameter) is stored on `SCEFit`
  only. Converting to `SCEModel` or saving to XML drops it, so the
  weighting used to produce a published model cannot be recovered.
...
- Add a `salc_fingerprint::UInt64` (or equivalent) field to `SCEBasis`
  ...
  `salcbasis.salc_list`.
- Serialize the fingerprint in the basis / model XML schema and compare
  it (alongside the existing `===` fast-path) inside `_check_basis`.
- Validate unit-norm spin directions in `SpinConfig`'s inner constructor.
- Persist `torque_weight` on `SCEModel` and in model XML, defaulting to
  `1.0` for older XML files.
...
- Any change to SALC ordering itself or to the existing XML payload
  beyond the new fields described above.

## Invariants

- Existing XML round-trips remain byte-identical on the payload that was
  already serialized; the new `salc_fingerprint` and `torque_weight`
  fields are added as new optional XML attributes / elements, and older
  XML files without them must still load.
...
- [ ] XML round-trip test verifies that `salc_fingerprint` and
      `torque_weight` survive `save` / `load`, and that an older XML
      without these fields still loads (with `torque_weight = 1.0`).
- [ ] `CHANGELOG.md` `[Unreleased]` updated; if XML schema gained
      fields, note as non-breaking additive change.
...
- Source touchpoints:
  ...
  - `src/Magesty.jl:161, 1234, 1241` (existing 4-arg `SCEBasis(...)`
    construction sites)
  - `src/Magesty.jl:265-278` (`SCEModel` struct)
  ...
  - `src/XMLIO.jl` (basis / model XML schema)
