...
| [260618-sunny-physical-spin](260618-sunny-physical-spin/) | draft (2026-06-18) | `sce_to_sunny` gains a required `spin` (effective spin length `S_eff = m/(gμ_B)`, scalar or per-species `Dict`) plus `g` / `mode`: bonds rescaled by `1/(s_i s_j)`, single-ion by mode (`:dipole` `2/(s(2s-1))` / `:dipole_uncorrected` `1/s²`), so the LSWT magnon dispersion is physical (fixes the ~S-times-too-large frequencies; static energy stays invariant). |
