...
| [260618-sunny-physical-spin](260618-sunny-physical-spin/) | implemented (2026-06-18; `:coupling` route 2026-06-24) | `sce_to_sunny` gains a required `spin` (effective spin length `S_eff = m/(gμ_B)`, scalar or per-species `Dict`) plus `g` / `mode` / `scaling`: bonds rescaled by `1/(s_i s_j)`, single-ion by mode (`:dipole` `2/(s(2s-1))` / `:dipole_uncorrected` `1/s²`), so the LSWT magnon dispersion is physical (fixes the ~S-times-too-large frequencies; static energy stays invariant). The follow-up `scaling = :coupling` route carries `S_eff` in the couplings with a placeholder `Moment` (`s₀ = 1`) so itinerant / non-half-integer moments get a physical dispersion (dispersion-only; static energy rescaled). |
| [260702-salc-projection-timerev-dedup](260702-salc-projection-timerev-dedup/) | implemented (2026-07-02) | Deduplicate the `time_rev_sym`-independent work in `_projection_matrix_coupled_basis` (single-pass dual-buffer loop). Bit-identity gate PASS; projection stage 1.95x / -47% allocs, constructor 1.19x / -29% allocs on fege_2x2x2 |
