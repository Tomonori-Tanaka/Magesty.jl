...
| 段階 | 現名 | 中身 | 問題 |
|---|---|---|---|
| ① セットアップ | `System` | `Structure`+`Symmetry`+`Cluster`+`SALCBasis` | `System` は generic すぎて `ModelingToolkit.System` 等と衝突しうる |
...
| 型 | 役割 | 中身（概念） | 重さ |
|---|---|---|---|
| `SCEProblem` | 物質+基底の静的セットアップ | `Structure`+`Symmetry`+`Cluster`+`SALCBasis` | 軽量 |
...
1. `SCEProblem` 型と Julia kwargs constructor の追加（既存 `Structure`/`Symmetry`/`Cluster`/`SALCBasis` を内部で構築する薄いラッパー）。
