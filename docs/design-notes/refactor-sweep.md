...
`System(input_dict)`（L132–148）、`build_sce_basis`（L172–179）、`SpinCluster(input_dict)`（L286–307）の 3 箇所で、`Config4System → Structure → Symmetry → Cluster → SALCBasis` の同じ 5 ステップが繰り返されていた。さらに `build_sce_basis_from_xml`（L205–230）は SALCBasis 部分だけ XML 読み込みに置換した 4 つ目のバリアント。

**実装**: private helper `_build_structure_skeleton(config; verbosity)` を抽出し `(structure, symmetry, cluster)` を返す形に。SALCBasis は caller が 1 行で追記する（XML パスは置換しやすい）。当初案の `basisset_override` kwarg は採用せず、caller 側で分岐する方が `build_sce_basis_from_xml` の verbose 出力順序が保たれる。
...
### R3. `SALCBases.jl:554` の TODO とコメントアウト処理（既知バグ）

**Status**: **完了** (commit `833b153`, 2026-05-13). branch なし → main に直接 fast-forward。

**対象**: `src/SALCBases.jl` `listup_coupled_basislist` 周辺
...
**実装**: 2-atom BCC + `body2.lsum = 4` で `l_vec ∈ {[1,3], [2,2], [3,1]}` を経由する回帰テストを `test/component_test/test_BasisSets_l13_regression.jl` に新設し、`projection_matrix_coupled_basis` まで含めた full SALCBasis build が通ることを確認。その後コメントアウトコードと TODO を削除。
...
`OffsetArrays` パッケージ依存自体は `Clusters.jl` / `SALCBases.jl` / `ConfigParser.jl` が使うため Project.toml からは外せない。
...
**連動箇所**: 球面調和関数の規格化・符号は変更なし → `SphericalHarmonicsTransforms.jl` / `SALCBases.jl` / `test_sphericart_agreement.jl` への影響なし。テスト: `test-unit` 6203/6203、`test-integration` 155/155、`test-jet`、`test-aqua` 全パス。
...
- **`SALCBases.jl` legacy SHProduct path 群**: ~~`construct_basislist`（L957–1000 付近）~~ **完了** (Phase 1 + Phase 2, 2026-05-14). Phase 1: `construct_basislist` を起点とする 9 関数 (`push_unique_body!`, `listup_basislist`, `map_atom_l_list`, `classify_basislist`, `is_translationally_equiv_basis`, `projection_matrix` (legacy), `proj_matrix_a_symop`, `operate_symop`, `corresponding_idx`) を `SALCBases.jl` から削除 (~500 行)。Phase 2: 連動して `AtomicIndices` モジュール全体が dead になったので、`src/types/AtomicIndices.jl` (217 行) と `test/component_test/test_AtomicIndices.jl` (249 行) を削除。`SHSiteIndex` / `SHProduct` / `LinearCombo` / `inner_product` / `replace_atom` / `product_shsiteindex` / `get_atom_l_list` / `shsiteindex_singleatom` がすべて消滅。SALCBases / Optimize / Magesty / runtests の `using ..AtomicIndices` / `include` も同時に整理。
- **`SpinConfigs.show`（L175–197）と `Clusters.print_cluster_stdout`（L506–566）**: ~~出力フォーマットの統一余地。~~ **完了** (2026-05-14). 「両者を同じパターン」は性質が違う (struct の rich display vs cluster 構築時の診断ログ) ため統一せず、それぞれ独立に Julia 慣習へ寄せた: (1) `SpinConfigs.show` を `show(io, config::SpinConfig)` 2-arg から `show(io, ::MIME"text/plain", config::SpinConfig)` 3-arg に変更し、REPL の rich display は維持しつつ `print(config)` / `string(config)` / `Vector{SpinConfig}` 表示時はデフォルトの compact 表現に。(2) `print_cluster_stdout` を `(io::IO, ...)` シグネチャに変更し、io 省略時 stdout にディスパッチする wrapper を追加。`println` / `@printf` / `print` を全 io 経由に統一。`PrettyTables.jl` 新規依存は検討せず据置 (現状の整形で十分機能)。
