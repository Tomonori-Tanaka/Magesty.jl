...
struct DataSet
	...
	function DataSet(
		...
		if maximum(training_data_indices) > length(spinconfigs) || maximum(validation_data_indices) > length(spinconfigs)
			throw(ArgumentError("Indices exceed the number of configurations"))
		end
