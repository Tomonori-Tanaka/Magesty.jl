...
- **GCV diagnostics (spec 260610-gcv-diagnostics).** New exported
  ...
  score for a fit; `gcv_lambda(dataset, lambdas; torque_weight)` sweeps the
  ...
  non-linear estimators raise `ArgumentError`. `write_gcv_lambda` /
  `write_gcv_learning_curve` write the sweeps to text for the new
  `tools/FitCheck_gcv_lambda.py` / `tools/FitCheck_gcv_learning_curve.py` plotters.
  No change to existing fit results.
